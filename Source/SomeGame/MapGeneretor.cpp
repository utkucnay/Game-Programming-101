...
void AMapGeneretor::BeginPlay()
{
	...
	auto noise = PerlinNoiseGeneretor::GetPerlinNoiseSquare(mapLenght, FMath::SRand() * 20);
	
	for (int y = 0; y < mapLenght - 1; y++) {
		for (int x = 0; x < mapLenght - 1; x++)
		{
			SpawnObjectInRange<ATree>(.3f, .305f, noise[y * mapLenght + x].X,
				FVector(x * 100 + 50, y * 100 + 50, 0), FRotator(0, noise[y * mapLenght + x].Y, 0));

			SpawnObjectInRange<ATree>(-.305f, -.3f, noise[y * mapLenght + x].X,
				FVector(x * 100 + 50, y * 100 + 50, 0), FRotator(0, noise[y * mapLenght + x].Y, 0));
